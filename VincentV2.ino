...
//20200912 Mise à jour chez Vincent avec rajout de la partie Air
...
const String Recettes[] = {"1-Margarita      ", "2-Cognac Coca     ", "3-Rhum Coca      ", "4-Cosmopolitan      ","5-Blue Lagoon      ","6-Daiquiri      ","7-Long Island Ice Tea     ","8-Coca Cola       ","9-Cuba Libre     ","10-Black Russian       ", "11-Screwdriver     ", "12-42nd Street  ", "13-Orange Blossom  ", "14-Bourbon Sour   ", "15-Bourbon      ", "16-Tequila Sunrise  ", "17-Red Lion     ", "18-Rainbow Shot The Sheriff  ", "19-Kentucky Road  ", "20-White Lady   ", "21-Ti Punch       ", "22-Cognac Tonic   ", "23-Ponche Caribenu      ", "24-Bacardi Cocktail   ", "25-BlackD         ", "26-Between Sheets", "RETOUR             "}; // Tableau de recette
//const String Recettes[13] = {"1-Ti Punch      "}; // Tableau de recette
const int nombreDeRecettes = 27; // Permet le modula pour revenir à 1 en cycle navigation (+1 pour RETOUR)
const String Pompes[] = {"Coca                ", "Jack Daniel's   ", "Cognac             ", "Vodka           ", "Rhum Blanc      ", "Gin           ", "Bacardi         ", "Canadou       ", "Jus d'Orange    ", "Tequila          ", "Cointreau    ", "Curacao Bleu    ", "Citron Vert   ", "Cranberry   ", "Vermouth Dry   ", "Citron           ", "Rye Whisky      ", "Kahlua       ", "Grenadine       ", "Invite Mystere     ","Schweppes                ", "RETOUR            "}; // Affichage du menu pour la distribution
//const String Pompes[] = {"Pompe 1 ", "Pompe 2 "}; // Affichage du menu pour la distributionrnotepad
const int nombreDePompes = 22; // Constante du nombre de pompe pour le modulo (+1 pour RETOUR)
...
const int cranberry = 24; // Pin pour la board relais
const int vodka = 25; // Pin pour la board relais
const int citronv = 26; // Pin pour la board relais
const int gin = 27; // Pin pour la board relais
const int citron = 28; // Pin pour la board relais
...
const int rhum = 34; // Pin pour la board relais
const int cognac = 35; // Pin pour la board relais
const int vermouth = 36; // Pin pour la board relais
const int bacardi = 37; // Pin pour la board relais
const int schweppes = 38; // Pin pour la board relais
...
void setup() {
  ...
  pinMode(38, OUTPUT); // Déclaration des pins de pompe comme sortie pour la board relais
  ...
  switch (mode) {
    ...
    case 4: // Sous menu pour le nombre de Cl pour le Shaker
     ...
     if (etatBout2) { // Si le bouton 2 (BAS) est appuyé
      ...
      // Il faut changer la façon de gérer la boucle en fonction du besoin (pompe ou pompeairà lié à la variable while en fonction de la boucle. Donc 2 if qui gère 2 boucles différentes
      // DEBUT DU RAJOUT
      if (lapompe == 22 || lapompe == 38) {
        do // On lance une boucle qui ne s'arrêtera qu'à la fin du chargement.
      {
       avancement = ((millis() - temps)/convair(posMenu+1)*100); // Le "temps" posMenu+1 correspond à la position dans le menu lors du choix du nombre de CL. Si on choisi 4CL, alors on demande la position (3) +1. La fonction conv(4) permet ensuite de transformer cela en temps.
       draw_progressbar(avancement);
              // Si le numéro de la pompe (lapompe) est du pin 22 ou 38, nous sommes sur les pompes à air, donc la fonction à utiliser est pompeV2air et non pompeV2
       pompeV2air(lapompe, posMenu+1); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer. 
       //Serial.println((millis() - temps));
      } while ((millis() - temps) < convair(posMenu+1)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide
      }
      else { // else = gestion de la boucle différente
       do
       {
    ...
    case 5: // Sous menu pour le chargement unitaire des pompes.
     ...
     if (etatBout2) { // Si le bouton 2 (BAS) est appuyé
      ...
      if (lapompe == 22 || lapompe == 38) {
        do // On lance une boucle qui ne s'arrêtera qu'à la fin du chargement.
      {
       avancement = ((millis() - temps)/convair(1)*100); // Le "temps" posMenu+1 correspond à la position dans le menu lors du choix du nombre de CL. Si on choisi 4CL, alors on demande la position (3) +1. La fonction conv(4) permet ensuite de transformer cela en temps.
       draw_progressbar(avancement);
              // Si le numéro de la pompe (lapompe) est du pin 22 ou 38, nous sommes sur les pompes à air, donc la fonction à utiliser est pompeV2air et non pompeV2
       pompeV2air(lapompe, 1); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer. 
       //Serial.println((millis() - temps));
      } while ((millis() - temps) < convair(1)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide
      }
      else { // else = gestion de la boucle différente
       do
       {
        avancement = ((millis() - temps)/conv(1)*100); // Le "temps" posMenu+1 correspond à la position dans le menu lors du choix du nombre de CL. Si on choisi 4CL, alors on demande la position (3) +1. La fonction conv(4) permet ensuite de transformer cela en temps.
       draw_progressbar(avancement);
              // Si le numéro de la pompe (lapompe) est du pin 22 ou 38, nous sommes sur les pompes à air, donc la fonction à utiliser est pompeV2air et non pompeV2
    ...
    case 6: // Sous menu pour la purge unitaire des pompes.
     ...
     if (etatBout2) { // Si le bouton 2 (BAS) est appuyé
      ...
      if (lapompe == 22 || lapompe == 38) {
        do // On lance une boucle qui ne s'arrêtera qu'à la fin du chargement.
      {
       avancement = ((millis() - temps)/convair(4)*100); // Le "temps" posMenu+1 correspond à la position dans le menu lors du choix du nombre de CL. Si on choisi 4CL, alors on demande la position (3) +1. La fonction conv(4) permet ensuite de transformer cela en temps.
       draw_progressbar(avancement);
              // Si le numéro de la pompe (lapompe) est du pin 22 ou 38, nous sommes sur les pompes à air, donc la fonction à utiliser est pompeV2air et non pompeV2
       pompeV2air(lapompe, 4); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer. 
       //Serial.println((millis() - temps));
      } while ((millis() - temps) < convair(4)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide
      }
      else { // else = gestion de la boucle différente
       do
       {
        avancement = ((millis() - temps)/conv(4)*100); // Le "temps" posMenu+1 correspond à la position dans le menu lors du choix du nombre de CL. Si on choisi 4CL, alors on demande la position (3) +1. La fonction conv(4) permet ensuite de transformer cela en temps.
       draw_progressbar(avancement);
              // Si le numéro de la pompe (lapompe) est du pin 22 ou 38, nous sommes sur les pompes à air, donc la fonction à utiliser est pompeV2air et non pompeV2
  ...
  switch (posMenu) { // En fonction de la position dans le menu lors de la selection, on charge les pompes
     ...
     case 1: // Bourbon Coca
      ...
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       ...
       draw_progressbar(avancement);
       pompeV2air(coca,3); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2(jack,3);
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      strip.setPixelColor(i, Wheel((i+j) & 255)); // On attribue une couleur
     ...
     case 7: // Coca
      ...
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       ...
       draw_progressbar(avancement);
       pompeV2air(coca,1); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      strip.setPixelColor(i, Wheel((i+j) & 255)); // On attribue une couleur
      }
     ...
     case 14: // Bourbon 
      //On allume toutes les pomptes du cocktails
      digitalWrite(cognac, LOW);
      digitalWrite(coca,LOW);
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       avancement = ((millis() - temps)/conv(6)*100);
       draw_progressbar(avancement);
       pompeV2(cognac,2); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2air(coca,6); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      strip.setPixelColor(i, Wheel((i+j) & 255)); // On attribue une couleur
      }
     ...
     case 20: // Ti Punch
      ...
      break;
     //"22-Cognac Tonic   ", "23-Ponche Caribenu      ", "24-Bacardi Cocktail   ", "25-BlackD         ", "26-Between Sheets"
     case 21: // Cognac Tonic
      //On allume toutes les pomptes du cocktails
      digitalWrite(cognac, LOW);
      digitalWrite(schweppes, LOW);
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       avancement = ((millis() - temps)/conv(6)*100);
       draw_progressbar(avancement);
       pompeV2(cognac,2); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2air(schweppes,6);
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      strip.setPixelColor(i, Wheel((i+j) & 255)); // On attribue une couleur
      }
      ...
      // Fin du rainbow maison   
      } while ((millis() - temps) < conv(6)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide (ici rhum)
      break;
     case 22: // Ponche Caribenu 7jus dorange 3cl de rhum blanc 1cl de jus de citron
      //On allume toutes les pomptes du cocktails
      digitalWrite(orange, LOW);
      digitalWrite(rhum, LOW);
      digitalWrite(citron, LOW);
      ...
      // Fin du rainbow maison   
      } while ((millis() - temps) < conv(7)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide (ici rhum)
      break;
     case 23: // Bacardi Cocktail 0,5 grenadine 2 Cl de Citron 4 Cl de Bacardi
      //On allume toutes les pomptes du cocktails
      digitalWrite(grenadine, LOW);
      digitalWrite(citron, LOW);
      digitalWrite(bacardi, LOW);
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       avancement = ((millis() - temps)/conv(4)*100);
       draw_progressbar(avancement);
       pompeV2(grenadine,0.5); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2(citron,2);
       pompeV2(bacardi,4);
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      ...
      // Fin du rainbow maison   
      } while ((millis() - temps) < conv(4)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide (ici rhum)
      break;
     case 24: // BlackD 6Cl de Bacardi 15Cl de Coca 0,5 Grenadine
      //On allume toutes les pomptes du cocktails
      digitalWrite(bacardi, LOW);
      digitalWrite(coca, LOW);
      digitalWrite(grenadine, LOW);
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       ...
       draw_progressbar(avancement);
       pompeV2(bacardi,6); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2air(coca,15);
       pompeV2(grenadine,0.5);
      // Début du rainbow maison
      for(i=0; i<strip.numPixels(); i++) { // Pour chaque led du Ring, 
      strip.setPixelColor(i, Wheel((i+j) & 255)); // On attribue une couleur
      }
      ...
      // Fin du rainbow maison   
      } while ((millis() - temps) < conv(15)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide (ici rhum)
      break;
     case 25: // Between Sheets 2cl de rhum, 2cl de cognac, 1cl de cointreau, 1cl de citron
      //On allume toutes les pomptes du cocktails
      digitalWrite(rhum, LOW);
      digitalWrite(cognac, LOW);
      digitalWrite(cointreau, LOW);
      ...
      do // On lance une boucle qui ne s'arrêtera qu'à la fin du liquide le plus présent.
      {
       avancement = ((millis() - temps)/conv(2)*100);
       draw_progressbar(avancement);
       pompeV2(rhum,2); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
       pompeV2(cognac,2);
       pompeV2(cointreau,1);
       pompeV2(citron,1);
      // Début du rainbow maison
      ...
      // Fin du rainbow maison   
      } while ((millis() - temps) < conv(2)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide (ici rhum)
      break;
      default: // Sinon
      monEcran.setCursor(0, 1); // On postionne le curseur sur la deuxième ligne
...
void distrunique() {
  ...
  do // On lance une boucle qui ne s'arrêtera qu'à la fin du chargement.
  {
   avancement = ((millis() - temps)/conv(4)*100);
   draw_progressbar(avancement);
   pompeV2(whisky,4); //  Si la Pompe est activée, alors on regarde si elle a pu délivrer x Cl ( x CL = X * 6000 millisecondes). Si c'est le cas, alors on coupe la pompe, sinon on laisse allumer.
   //Serial.println((millis() - temps));
   } while ((millis() - temps) < conv(4)); // Tant qu'on ne dépasse pas le temps nécessaire au plus grand liquide
...
void chargement() {
  ...
  do {
    ...
    digitalWrite(i, HIGH);
      ...
      digitalWrite(22,LOW); //Chargement du tuyau bulle coca
  temps = millis(); // On sauvegarde l'heure actuel
  do {
      // Chargement des tuyaux, on patiente
      // On allume les lumières de chargement en attendant
   ...
   digitalWrite(22, HIGH);
      ...
      digitalWrite(38,LOW); //Chargement du tuyau bulle schweppes
  temps = millis(); // On sauvegarde l'heure actuel
  do {
      // Chargement des tuyaux, on patiente
      // On allume les lumières de chargement en attendant
...
void purge() {
  ...
  do {
    ...
    digitalWrite(i, HIGH);
      ...
      digitalWrite(22,LOW); //DeChargement du tuyau bulle coca
  temps = millis(); // On sauvegarde l'heure actuel
  do {
      // DeChargement des tuyaux, on patiente
      // On allume les lumières de chargement en attendant
      colorLED(strip.Color(250, 0, 0), 50); // Simulation de DeChargement
     } while ((millis() - temps) < 5000); // Tant qu'on n'a pas rempli les tuyaux en 3 sec, on laisse allumé.
   digitalWrite(22, HIGH);

   digitalWrite(38,LOW); //DeChargement du tuyau bulle schweppes
  temps = millis(); // On sauvegarde l'heure actuel
  do {
      // DeChargement des tuyaux, on patiente
      // On allume les lumières de chargement en attendant
